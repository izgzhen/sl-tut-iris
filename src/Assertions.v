...
Section basics.
  ...
  Definition foo : iProp Σ := (x ↦ #1)%I.
  (* [%I] is Iris scope, which types this expression as [iProp Σ]; [#] is used to transfrom literal to a general value *)

  (** A simple example: [foo] defines a singleton heap from [x] to [1],
      which means that there is a singleton heap in which [x] points to integer literal *)
...
As said in the paper _Interactive Proofs in Higher-Order Concurrent Separation Logic_,
